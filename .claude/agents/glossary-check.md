...
# Role
Read a changed markdown file. Compare its terminology against the domain's `glossary.md` and Foundation's repo-wide `glossary.md`.
...
- Prefer extending the domain's glossary; only escalate to Foundation's `glossary.md` if the term spans ≥2 domains.
